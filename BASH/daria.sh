...
do
	mkdir $chap
	cd $chap
	aria2c -o index.html -l log.txt -x 4 -q http://www.mangareader.net/$manga/$chap
	...
	do
		...
		aria2c -x 4 -l log.txt -o $i.html  -q http://www.mangapanda.com/$manga/$chap/$i # Downloads the main webpage
		...
		imagename=0000$i
		aria2c -o ${imagename: -4}.jpg -l log.txt -q -x 4  $image
	done
